...
public class NPSheepSpawner : MonoBehaviour
{
	...
	void Start()
	{
		...
		if (npSheepToSpawn > ObjectPooler.Instance.pooledAmount)
			npSheepToSpawn = ObjectPooler.Instance.pooledAmount;
	}
	...
	public void SpawnNPSheep()
	{
		...
		for (int i = 0 ; i < npSheepToSpawn ; i ++)
		{
			...
			GameObject npSheep = ObjectPooler.Instance.GetPooledObject ();
